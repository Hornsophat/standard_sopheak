...
@section('title',"Edit Expense Group")
@section('style')
    ...
    <main class="app-content">
        <div class="app-title">
            <ul class="app-breadcrumb breadcrumb side">
                ...
                <li class="breadcrumb-item active"><a href="#">{{ __('item.edit_expense_type') }}</a></li>
            </ul>
        </div>
        <div class="col-lg-12">
            @include('flash/message')
            <div class="panel-body bg-white rounded overflow_hidden p-4">
                <h3 class="text-dark mb-4">{{ __('item.edit_expense_type') }}</h3><hr/>

                <form action="{{ route('expense_group.edit',['id'=>$group->id]) }}" method="POST" class="row form-horizontal" enctype="multipart/form-data">
                    {{ csrf_field() }}
                    <!-- First name -->
                    <div class="col-lg-6 offset-lg-3 form-group d-lg-flex align-items-center{{ $errors->has('name') ? ' has-error' : '' }}">
                        ...
                        <div class="col-lg-9 p-0">
                            <input type="text" name="name" class="form-control" value="{{$group->name}}" required autofocus>
                            @if ($errors->has('name'))
