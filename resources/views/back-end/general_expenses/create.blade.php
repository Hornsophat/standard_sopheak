	...
	<main class="app-content">
		<div class="app-title">
	        <ul class="app-breadcrumb breadcrumb side">
	          	...
	          	<li class="breadcrumb-item active"><a href="#">{{ __('item.add_expense') }}</a></li>
	        </ul>
      	</div>
        <div class="col-lg-12">
            @include('flash/message')
          	<div class="panel-body bg-white rounded overflow_hidden p-4">
          		<h3 class="text-dark mb-4">{{ __('item.add_expense') }}</h3><hr/>

                <form action="{{ route('general_expense.create') }}" method="POST" class="row form-horizontal" enctype="multipart/form-data">
                    {{ csrf_field() }}
                        ...
                        <div class="row">
                            <div class="col-md-12 form-group d-lg-flex align-items-center{{ $errors->has('date') ? ' has-error' : '' }}">
                                ...
                                <div class="col-lg-9 p-0">
                                    <input type="text" name="date" class="form-control demoDate" value="{{ date('d-m-Y') }}" id="date" required >
                                    @if ($errors->has('date'))
                            ...
                            <div class="col-md-12 form-group d-lg-flex align-items-center{{ $errors->has('amount') ? ' has-error' : '' }}">
                                ...
                                <div class="col-lg-9 p-0">
                                    <input type="text" id="amount" name="amount" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" class="form-control" min="0" value="{{old('amount')}}" required>
