...
@section('title',__('item.add_public_holiday'))
@section('style')
	...
	<main class="app-content">
		<div class="app-title">
	        <ul class="app-breadcrumb breadcrumb side">
	          	...
	          	<li class="breadcrumb-item active"><a href="#">{{ __('item.add_public_holiday') }}</a></li>
	        </ul>
      	</div>
        <div class="col-lg-12">
            @include('flash/message')
          	<div class="panel-body bg-white rounded overflow_hidden p-4">
          		<h3 class="text-dark mb-4">{{ __('item.add_public_holiday') }}</h3><hr/>

                <form action="{{ route('public_holiday.create') }}" method="POST" class="row form-horizontal" enctype="multipart/form-data">
                    {{ csrf_field() }}
                    <!-- First name -->
                    ...
                    <div class="col-lg-6 offset-lg-3 form-group d-lg-flex align-items-center{{ $errors->has('date') ? ' has-error' : '' }}">
                        ...
                        <div class="col-lg-9 p-0">
                            {{ Form::text('date', old('date')??'', ['class' => 'width-100 form-control demoDate', 'autocomplete' => 'off', 'placeholder' => 'Date', 'required']) }}
                            @if ($errors->has('date'))
    	                        <span class="help-block text-danger">
